...
carrosseis.forEach((carousel) => {
    ...
    if (!produtos || !btnAnterior || !btnProximo) {
...
cards.forEach((card) => {
    ...
    card.addEventListener("click", () => {
        ...
        if (!modal || !modalNome || !modalDescricao || !modalPreco || !modalImagem) {
...
function facaAgora(){
    window.location.href = "/html/principalPagina/finalizar.html"
}
...
function atualizarQuantidadeCarrinho() {
    ...
    const pedidos =
        JSON.parse(
            localStorage.getItem("meusPedidos")
        ) || [];
